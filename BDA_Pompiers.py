...
@st.cache_data
def load_data():
    df_incidents_1820 = pd.read_csv("LFB Incident data from 2018-2020.zip", compression='zip', encoding='ISO-8859-1', sep=',', on_bad_lines='skip')
    df_incidents_2122 = pd.read_csv("LFB Incident data from 2021-2022.zip", compression='zip', encoding='ISO-8859-1', sep=',', on_bad_lines='skip')
    df_incidents_2324 = pd.read_csv("LFB Incident data from 2023-2024.zip", compression='zip', encoding='ISO-8859-1', sep=',', on_bad_lines='skip')
    df_mob_1520 = pd.read_csv("LFB Mobilisation data from 2015 - 2020.zip", compression='zip', encoding='ISO-8859-1', sep=';', on_bad_lines='skip', low_memory=False)
    df_mob_2124 = pd.read_csv("LFB Mobilisation data 2021 - 2024.zip", compression='zip', encoding='ISO-8859-1', sep=';', on_bad_lines='skip', low_memory=False)
    ...
    df_incidents = pd.concat([df_incidents_1820, df_incidents_2122, df_incidents_2324], ignore_index=True)
    return df_incidents, df_mob_1520, df_mob_2124, df_final

df_incidents, df_mob_1520, df_mob_2124, df_final = load_data()
...
# Sélection du jeu de données
    ...
    if dataset_choice == "Incidents (2018-2024)":
        ...
        st.markdown(f"**Taille du DataFrame** : {df_incidents.shape[0]} lignes x {df_incidents.shape[1]} colonnes")
        ...
        st.write(df_incidents.describe())

        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**Valeurs manquantes :**")
            missing_values = pd.DataFrame({
                "Colonnes": df_incidents.columns,
                "NaNs": df_incidents.isna().sum()
            }).reset_index(drop=True)
            missing_values["Pourcentage"] = (missing_values["NaNs"] / len(df_incidents) * 100).map("{:.2f}%".format)
            st.dataframe(missing_values.style
                        .set_properties(subset=["Colonnes"], **{'width': '350px'})
                        .set_properties(subset=["NaNs"], **{'width': '200px'}))
        with col2:
        ...
        st.subheader("3. Exploration des données")
        st.markdown("**Langage utilisé** : Python")
        st.markdown(f"**Taille du DataFrame** : {df_mob_1520.shape[0]} lignes x {df_mob_1520.shape[1]} colonnes")
        st.markdown("**Les premières lignes de ce jeu de données :**")
        st.write(df_mob_1520.head())
        st.markdown("**Résumé statistique de tout le jeu de données :**")
        st.write(df_mob_1520.describe())

        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**Valeurs manquantes :**")
            missing_values = pd.DataFrame({
                "Colonnes": df_mob_1520.columns,
                "NaNs": df_mob_1520.isna().sum()
            }).reset_index(drop=True)
            missing_values["Pourcentage"] = (missing_values["NaNs"] / len(df_mob_1520) * 100).map("{:.2f}%".format)
            st.dataframe(missing_values.style
                        .set_properties(subset=["Colonnes"], **{'width': '350px'})
                        .set_properties(subset=["NaNs"], **{'width': '200px'}))
        with col2:
        ...
        st.subheader("3. Exploration des données")
        st.markdown("**Langage utilisé** : Python")
        st.markdown(f"**Taille du DataFrame** : {df_mob_2124.shape[0]} lignes x {df_mob_2124.shape[1]} colonnes")
        st.markdown("**Les premières lignes de ce jeu de données :**")
        st.write(df_mob_2124.head())
        st.markdown("**Résumé statistique de tout le jeu de données :**")
        st.write(df_mob_2124.describe())

        col1, col2 = st.columns(2)
        with col1: 
            st.markdown("**Valeurs manquantes :**")
            missing_values = pd.DataFrame({
                "Colonnes": df_mob_2124.columns,
                "NaNs": df_mob_2124.isna().sum()
            }).reset_index(drop=True)
            missing_values["Pourcentage"] = (missing_values["NaNs"] / len(df_mob_2124) * 100).map("{:.2f}%".format)
            st.dataframe(missing_values.style
                        .set_properties(subset=["Colonnes"], **{'width': '350px'})
                        .set_properties(subset=["NaNs"], **{'width': '200px'}))
        with col2:
        ...
        st.markdown("**Langage utilisé** : Python")
        st.markdown(f"**Taille du DataFrame** : {df_final.shape[0]} lignes x {df_final.shape[1]} colonnes")
        st.markdown("**Les premières lignes de ce jeu de données :**")
        ...
        with col1:
            ...
            missing_values["Pourcentage"] = (missing_values["NaNs"] / len(df_final) * 100).map("{:.2f}%".format)
            st.dataframe(missing_values.style
                        .set_properties(subset=["Colonnes"], **{'width': '350px'})
                        .set_properties(subset=["NaNs"], **{'width': '200px'}))
        with col2:
